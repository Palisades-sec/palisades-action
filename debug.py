...
from langchain.tools import StructuredTool, Tool, WikipediaQueryRun, tool
...
llm = ChatOpenAI(
    ...
    openai_api_key="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",
)
...
repository_name = "Palisades-sec/palisades-feature-api"
cf_auth_token = "Ps-1Ca21C0A-42Cc-42D4-94C4-C72367234C05"
token = "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"
headers = {"Authorization": f"token {token}"}
...
# TODO Docstring,
@tool
def generate_code(issue_number: int):
...
# generate_code_tool = StructuredTool.from_function(generate_code)
...
# feature_development_agent = StructuredChatAgent(llm=llm, tools=tools, prompt=agent_prompt)
# agent_executor = AgentExecutor(agent=feature_development_agent, tools=tools, verbose=True)


# feature_development_agent.run(issue)
feature_development_agent.run(agent_prompt +'\nNew Issues created:\n'+ "Issue number 3")
